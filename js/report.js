...
function figContext(hostId) {
  const d = state.dist ? (state.dist.fam === 'given' ? suppliedName() : Dist.byId(state.dist.id).name + ' dissimilarity') : '';
  ...
  if (hostId.startsWith('fig6')) {
    const V = state.validation || {}, gen = { hier: 'tree-cut', kmeans: 'k-means', pam: 'PAM' };
    if (hostId === 'fig6Stab' && V.stability) return `${V.stability.partition}, ${V.stability.B} bootstrap samples`;
    if (hostId === 'fig6Pv' && V.pv) return `multiscale bootstrap of the variables, ${V.pv.B} resamples at ${V.pv.scales.length} scales`;
    if (hostId === 'fig6Ext' && V.external) return `${V.external.names[0]} against ${V.external.names[1]}`;
    if (hostId === 'fig6Cv' && V.algorithms) return 'hierarchical, k-means and PAM partitions over the k range of the validation';
    return V.sweep ? `validation over k = ${V.sweep.ks[0]}–${V.sweep.ks[V.sweep.ks.length - 1]} (${gen[V.sweep.generator] || V.sweep.generator} partitions on ${d})` : d;
  }
  if (hostId.startsWith('fig7')) { const P = state.profiles; return P ? `profiles of the ${P.k} clusters of the ${P.partition || 'adopted partition'}` : ''; }
  return '';
}
...
const wilksTxt = w => w < 0.001 ? w.toExponential(1) : w.toFixed(3);
const supplied = () => state.dist && state.dist.fam === 'given';
/* 'the supplied matrix', 'the square root of the supplied matrix' or 'the square of the supplied matrix' */
const suppliedName = () => ({ given_sqrt: 'the square root of the supplied dissimilarity matrix', given_sq: 'the square of the supplied dissimilarity matrix' })[state.dist.id] || 'the supplied dissimilarity matrix';
const noteTxt = s => { s = s.replace(/\.$/, '').replace(/\s*\(([^)]*)\)/g, ', $1'); return s.charAt(0).toLowerCase() + s.slice(1); };
const knownExt = () => state.validation && state.validation.external && state.validation.external.names.includes('known grouping') ? state.validation.external : null;
...
const jacWord = j => j > 0.85 ? 'highly stable' : j > 0.75 ? 'stable' : j > 0.6 ? 'only partly stable' : 'unstable';
Report.summaryTiles = () => {
  const t = [];
  ...
  if (state.dist) tile('Objects', state.dist.n, !state.isDistance && state.columns ? `${state.columns.filter(c => c.role === 'active').length} active variables` : 'dissimilarity matrix supplied');
  ...
  const ari = knownExt() ? knownExt().ari : (state.partition && state.partition.ariGroups != null ? state.partition.ariGroups : (state.hclust && state.hclust.ari != null ? state.hclust.ari : null));
...
Report.resultsText = () => {
  ...
  if (state.dist && state.dist.diag && state.dist.diag.euclid) { const e = state.dist.diag.euclid; p.push(`<b>Dissimilarity.</b> ${supplied() ? suppliedName().replace(/^t/, 'T') + ' was' : `The ${Dist.byId(state.dist.id).name} coefficient produced`} a ${e.negMass < 0.01 ? 'Euclidean-embeddable matrix, so principal coordinates and Ward linkage are directly applicable' : `non-Euclidean matrix (${fmtPct(e.negMass, 1)} of the eigenvalue mass is negative); ordinations and Ward linkage on it are approximate`}${state.dist.stress != null ? `. The two-dimensional principal-coordinate map reproduced the dissimilarities with a stress of ${state.dist.stress.toFixed(3)}` : ''}.`); }
  if (state.hclust) { const H = state.hclust, sz = sizes(H.cl); p.push(`<b>Hierarchical clustering.</b> ${HC_METHOD_NAMES[H.method]} linkage reproduced the dissimilarities with a cophenetic correlation of ${H.coph.toFixed(3)} (${H.coph > 0.8 ? 'faithful' : H.coph > 0.7 ? 'acceptable' : 'distorted'} tree) and ${H.method === 'diana' ? 'a divisive' : 'an agglomerative'} coefficient of ${H.coef.toFixed(3)}${H.coef > 0.8 && H.coph > 0.75 ? ', indicating clear structure' : ''}. Cutting the tree at k = ${H.k} gave clusters of ${sz.join(', ')} objects${H.ari != null ? ` (adjusted Rand index against the known grouping = ${H.ari.toFixed(3)})` : ''}${H.reversals ? `; ${H.reversals} reversal${H.reversals > 1 ? 's' : ''} occurred in the merge heights` : ''}.`); }
  if (state.partition) { const R = state.partition, sz = sizes(R.cluster); p.push(`<b>Partitioning.</b> ${R.name} with k = ${R.k} produced clusters of ${sz.join(', ')} objects${R.noise ? ` and left ${R.noise} objects as noise` : ''}. The average silhouette width was ${R.sil.avg.toFixed(3)}, which indicates ${silWord(R.sil.avg)}${R.sil.s ? `; ${(neg => neg === 0 ? 'no object has' : neg === 1 ? 'one object has' : neg + ' objects have')(R.sil.s.filter(v => v < 0).length)} a negative silhouette (closer to another cluster than to its own)` : ''}${R.ariTree != null ? `. Agreement with the hierarchical cut: ARI = ${R.ariTree.toFixed(3)}` : ''}${R.ariGroups != null ? `; with the known grouping: ARI = ${R.ariGroups.toFixed(3)}` : ''}.`); }
  if (state.validation) { const V = state.validation, s = []; if (V.sweep && V.sweep.consensus[0]) { const c = V.sweep.consensus[0], tot = Object.values(V.sweep.votes).filter(v => v.k != null).length; s.push(`${c.votes} of ${tot} criteria pointed to k = ${c.k}${V.sweep.consensus[1] ? ` (next: k = ${V.sweep.consensus[1].k} with ${V.sweep.consensus[1].votes})` : ''}${c.k === V.sweep.ks[V.sweep.ks.length - 1] ? ', the upper end of the range evaluated, which suggests no natural number of clusters' : ''}`); if (V.sweep.gap && V.sweep.gap.best === 1) s.push('the gap statistic found no cluster structure (k = 1)'); } if (V.stability) { const m = S.mean(V.stability.clusters.map(c => c.mean).filter(isFinite)); s.push(`the bootstrap Jaccard means (${V.stability.clusters.map(c => c.mean.toFixed(2)).join(', ')}) show that the partition is ${jacWord(m)}${V.stability.clusters.some(c => c.mean < 0.6) ? `; cluster${V.stability.clusters.filter(c => c.mean < 0.6).length > 1 ? 's' : ''} ${V.stability.clusters.filter(c => c.mean < 0.6).map(c => c.cluster).join(', ')} should not be over-interpreted` : ''}`); } if (V.pv) s.push(`multiscale bootstrap gave AU support values for the tree clusters (see the annotated dendrogram)`); if (V.external) s.push(`${V.external.names[0]} and ${V.external.names[1]} agreed at ARI = ${V.external.ari.toFixed(3)} and NMI = ${V.external.nmi.toFixed(3)} (χ² p ${V.external.pval < 0.001 ? '< 0.001' : '= ' + V.external.pval.toFixed(3)})`); if (s.length) p.push(`<b>Number of clusters and validation.</b> ${s.join('; ').replace(/^./, c => c.toUpperCase())}.`); }
  if (state.profiles) { const P = state.profiles, q = P.quant || []; const sig = q.filter(v => v.p < 0.05).length; const top = q.slice().sort((a, b) => b.eta2 - a.eta2).slice(0, 4); const desc = top.map(v => { const hi = v.clusters.reduce((b, c, i) => c.mean > (b.m == null ? -Infinity : b.m) ? { m: c.mean, i } : b, { m: null, i: -1 }); return `${v.name} (η² = ${v.eta2.toFixed(2)}, highest in cluster ${hi.i + 1})`; }); p.push(`<b>Cluster profiles.</b> ${sig} of ${q.length} quantitative variables differed among clusters (ANOVA, p &lt; 0.05; descriptive only for the variables used to form the clusters); the most discriminant were ${desc.join(', ')}.${P.indval && P.indval.length ? ` ${P.indval.filter(r => r.p < 0.05).length} indicator taxa were significant at p &lt; 0.05.` : ''}${state.lda ? ` A linear discriminant analysis separated the clusters (Wilks' Λ = ${wilksTxt(state.lda.wilks)}, p ${state.lda.pWilks < 0.001 ? '< 0.001' : '= ' + state.lda.pWilks.toFixed(3)}) and re-classified ${fmtPct(state.lda.loo, 1)} of the objects correctly under leave-one-out cross-validation${isFinite(state.lda.loo) && state.lda.accuracy - state.lda.loo > 0.15 ? ` (against ${fmtPct(state.lda.accuracy, 0)} on the training objects: the rule over-fits and should not be used to assign new objects without fewer variables)` : ''}${state.tree ? `; ${state.tree.rules.length} classification rules of depth ≤ ${state.tree.depth} describe the partition with ${fmtPct(state.tree.accuracy, 0)} accuracy` : ''}.` : ''}`); }
...
const CSS = `
body{font-family:system-ui,sans-serif;color:#1d1d2b;background:#fff;max-width:1100px;margin:0 auto;padding:28px 36px;line-height:1.5;font-size:14px}
...
.spark{display:none}.methods{background:#fbfbfe;border:1px solid #e3e3ee;border-radius:10px;padding:14px 18px;font-size:.92rem}.toc{columns:2;font-size:.9rem;list-style:none;padding-left:0}.toc li{margin-bottom:2px}.notes{white-space:pre-wrap}
...
Report.methodsText = () => {
  const p = [];
  if (!state.isDistance && state.rawRows && state.rawRows.length) {
  ...
  } else if (state.isDistance) {
    p.push(`A ${state.dist ? state.dist.n : ''}-object dissimilarity matrix (${esc(state.fileName || '')}) was supplied directly and used as it was.`);
    if (state.eda && state.eda.hopkins) { const H = state.eda.hopkins; p.push(`Clustering tendency was assessed with the Hopkins statistic on the classical multidimensional scaling coordinates of the matrix (H = ${H.H.toFixed(3)}, p ${H.pval < 0.001 ? '< 0.001' : '= ' + H.pval.toFixed(3)} against ${H.nsim} uniform reference sets).`); }
  }
  if (state.dist) { const d = state.dist, c = Dist.byId(d.id); p.push(`${supplied() ? (state.dist.id === 'given' ? 'The supplied matrix' : `The analysis used ${suppliedName()}, which`) : `Dissimilarities between objects were computed with the ${c.name} coefficient${d.note ? ' (' + noteTxt(d.note) + ')' : ''}`}${d.diag && d.diag.euclid ? `${supplied() ? ' was' : '; the matrix was'} ${d.diag.euclid.negMass < 0.01 ? '' : 'not '}Euclidean-embeddable (${fmtPct(d.diag.euclid.negMass, 1)} negative eigenvalue mass)` : ''}.`); }
  ...
  if (state.partition) { const R = state.partition; p.push(`A ${R.name} partition into ${R.k} clusters was obtained${R.space ? ' on the ' + R.space : ''}${R.params && R.params.algorithm ? ` (${({ hartigan: 'Hartigan–Wong', lloyd: 'Lloyd', macqueen: 'MacQueen' })[R.params.algorithm] || R.params.algorithm}${R.method === 'hkmeans' ? ' started from the centroids of the tree cut' : `, ${R.params.nstart} start${R.params.nstart === 1 ? '' : 's'}`})` : ''}${R.params && R.params.model ? ` (covariance model ${R.params.model}, selected by BIC)` : ''}${R.method === 'dbscan' ? ` (ε = ${fmtNum(R.params.eps, 4)}, minPts = ${R.params.minPts}, ${R.noise} noise objects)` : ''}; its average silhouette width was ${R.sil.avg.toFixed(3)}${R.ariTree != null ? ` and its agreement with the hierarchical cut ARI = ${R.ariTree.toFixed(3)}` : ''}.`); }
  ...
  if (state.profiles) { const P = state.profiles; p.push(`Clusters were characterised by one-way ANOVA and Kruskal–Wallis tests per variable and by test values of the cluster means against the overall mean${P.cats.length ? ', and by χ² tests on the categorical variables' : ''}${P.indval ? '; indicator species were identified with the Dufrêne–Legendre indicator value and a permutation test' : ''}.`); if (state.lda) p.push(`A linear discriminant analysis of the clusters (Wilks' Λ = ${wilksTxt(state.lda.wilks)}, p ${state.lda.pWilks < 0.001 ? '< 0.001' : '= ' + state.lda.pWilks.toFixed(3)}) classified ${fmtPct(state.lda.loo, 1)} of the objects correctly under leave-one-out cross-validation${state.tree ? `, and a classification tree of depth ${state.tree.depth} summarised the partition in ${state.tree.rules.length} rules` : ''}.`); }
