...
function figContext(hostId) {
  const d = state.dist ? (state.dist.fam === 'given' ? 'the supplied dissimilarity matrix' : Dist.byId(state.dist.id).name + ' dissimilarity') : '';
  ...
  if (hostId.startsWith('fig6')) {
    ...
    if (hostId === 'fig6Cv' && V.clValid) return 'hierarchical, k-means and PAM partitions over the k range of the validation';
