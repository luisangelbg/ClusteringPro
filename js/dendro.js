/* ClusteringPro — dendrogram studio and Block 4 figures (P4.*), built on the Fig engine.
...
P4.clusters = (T, cfg) => {
  ...
  const order = P4.order(T, cfg), map = new Map(); order.forEach(i => { if (!map.has(cl[i])) map.set(cl[i], map.size + 1); });
...
P4.dendrogram = (cfg, T) => {
  ...
  const { k, cl, cutH, order } = P4.clusters(T, cfg);
...
P4.heatmap = (cfg, T) => {
  ...
  const svg = Fig.svg(cfg.width, cfg.height, cfg.theme);
  const { k, cl, order } = P4.clusters(T, cfg);
  /* column tree */
...
window.Dendro = Dendro; window.P4 = P4;
